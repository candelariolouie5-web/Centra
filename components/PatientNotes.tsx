...
const PatientNotes = ({ patient }: any) => {
    ...
    return (
      <div className="mt-3 space-y-2">
        ...
        {prescriptions.map((rx: Prescription, idx: number) => (
          <div key={rx.id || idx} className="rounded-xl border border-amber-200 bg-amber-50/70 px-4 py-3">
            <p className="text-sm font-semibold text-amber-900">{getPrescriptionTitle(rx)}</p>
            {!!getPrescriptionMeta(rx) && <p className="text-xs text-amber-700">{getPrescriptionMeta(rx)}</p>}
            {rx.instructions?.trim() && <p className="mt-1 text-xs text-amber-800">{rx.instructions}</p>}
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div className="grid gap-3 sm:grid-cols-2 mt-2">
        {diagnosticImages.map((image, index) => (
          ...
          >
            <img src={image} alt={`Diagnostic image ${index + 1}`} className="mx-auto block max-h-48 w-full rounded object-contain shadow-sm" />
  ...
  const renderSoapNoteCard = (note: SoapNote, index: number) => {
    ...
    return (
      <div
        key={note.id || index}
        className={`rounded-xl border transition cursor-pointer ${isSelected ? "border-cyan-400 bg-cyan-50/70 ring-2 ring-cyan-200" : "border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm"}`}
            ...
            <div className="flex items-center gap-3">
              ...
              {note.createdAt && (
                <span className="text-xs text-slate-400">
                  {new Date(note.createdAt).toLocaleTimeString("en-PH", { hour: "2-digit", minute: "2-digit" })}
                </span>
              )}
            </div>
            {note.chiefComplaint && <span className="text-xs text-slate-500 truncate max-w-[200px]">{note.chiefComplaint}</span>}
          </div>
          {isSelected && (
            <div className="mt-3 space-y-3 border-t border-slate-200 pt-3">
              {note.chiefComplaint && (
                <div>
                  ...
                  <p className="text-sm text-slate-700 whitespace-pre-wrap">{note.chiefComplaint}</p>
                </div>
              )}
              {note.historyOfIllness && (
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">History of Present Illness</p>
                  <p className="text-sm text-slate-700 whitespace-pre-wrap">{note.historyOfIllness}</p>
                </div>
              )}
              {note.remarks && (
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Remarks</p>
              ...
              {note.diagnosis && (
                <div>
                  ...
                  <p className="text-sm text-slate-700 whitespace-pre-wrap">{note.diagnosis}</p>
              ...
              {note.followUp && (
                <div>
                  ...
                  <p className="text-sm text-slate-700 whitespace-pre-wrap">{note.followUp}</p>
                </div>
              )}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-slate-700">
              {soapNotes.length > 0 ? `${soapNotes.length} consultation(s) recorded` : "No consultations recorded yet"}
            </p>
            {loading && <p className="mt-1 text-xs text-blue-600">Loading consultations...</p>}
        ...
        {!loading && !error && soapNotes.length === 0 && (
          <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50/80 p-8 text-center">
            <p className="text-sm text-slate-500">No SOAP notes found for this patient.</p>
            <p className="mt-1 text-xs text-slate-400">Click the "Add SOAP Note" button to create the first consultation record.</p>
          </div>
        )}
        {!loading && soapNotes.length > 0 && (
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4" onClick={() => setPreviewImage(null)}>
          <div className="relative w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <button type="button" onClick={() => setPreviewImage(null)} className="absolute right-3 top-3 z-10 rounded-full bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow hover:bg-slate-100">Close</button>
            <div className="overflow-hidden rounded-2xl bg-white p-3 shadow-2xl">
              <img src={previewImage} alt="Diagnostic preview" className="max-h-[85vh] w-full rounded-xl object-contain" />
            </div>
          </div>
        </div>
      )}
      <SoapNoteModal open={isSoapModalOpen} onClose={() => { setIsSoapModalOpen(false); handleSoapNoteSaved(); }} patient={patient} onSaved={handleSoapNoteSaved} />
    </>
  );
};
