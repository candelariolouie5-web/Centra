...
export default function DashboardPage() {
  ...
  const fetchAppointmentData = async () => {
    try {
      ...
      else console.error("Failed to fetch appointment data", result);
    } catch(err) {
  ...
  const fetchTodayAppointments = useCallback(async () => {
    try {
      ...
      else console.error("Failed to fetch today appointments", result);
    } catch(err) {
      console.error("Network error fetching today appointments", err);
    }
  }, []);
  ...
  const fetchConsultationData = async () => {
    try {
      ...
      } else console.error("Failed to fetch consultation data", result);
    } catch(err) {
      console.error("Network error fetching consultation data", err);
    }
  };
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="px-6 py-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">Monitor clinic appointments and services.</p>
          </div>
          <input
            type="search"
            placeholder="Search..."
      ...
      <main className="p-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
              ...
              <div className="p-6">
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
                    <Pie data={consultationData} dataKey="value" cx="50%" cy="50%" innerRadius={60} outerRadius={90}>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
          <div className="px-6 py-4 border-b flex justify-between">
            <h3 className="font-semibold text-gray-900">Appointments Scheduled Today</h3>
            <button onClick={fetchTodayAppointments} className="text-indigo-600 text-sm">Refresh</button>
          </div>

          {todayAppointments.length===0?(
            <div className="p-10 text-center text-gray-400">No appointments scheduled today</div>
          ):(
                ...
                <tbody>
                  {todayAppointments.map((appointment)=>(
                    <tr key={appointment.id} className="border-t">
                      ...
                      <td className="px-6 py-3">{new Date(appointment.appointmentTime).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</td>
